...
def navigate_to(page: str):
    ...
    st.rerun()
=======
>>>>>>> 877ab3ed49aaa0cab9357198f7d172279c79db26
...
>>>>>>> 877ab3ed49aaa0cab9357198f7d172279c79db26
        ...
        <div class="mode-card">
            ...
            <div class="mode-description">Perfect for business users and analysts. Get interactive charts, metrics, and PDF reports. No coding required!</div>
        </div>
        """, unsafe_allow_html=True)
    with col2:
<<<<<<< HEAD
...
>>>>>>> 877ab3ed49aaa0cab9357198f7d172279c79db26
        ...
        <div class="mode-card">
            ...
            <div class="mode-description">For developers and ML engineers. Download complete Python package with model files, scripts, and tests.</div>
        </div>
        """, unsafe_allow_html=True)
...
def upload_data_section():
    ...
    uploaded_file = st.file_uploader("Drag and drop your CSV file here", type=['csv'], help="Upload a CSV file with your dataset")
    col1, col2 = st.columns(2)
    with col1:
<<<<<<< HEAD
    ...
    if st.button("🚀 Build ML Model", type="primary", use_container_width=True):
        with st.spinner("🤖 AI is analyzing your data and building models..."):
            try:
                ...
                charts = report_gen.generate_visualizations(metrics=result['metrics'], feature_importance=result.get('feature_importance', pd.DataFrame()), predictions=result.get('predictions', pd.DataFrame()), task_type=result['task_type'])
                ...
                report_gen = ReportGenerator()
                charts = report_gen.generate_visualizations(metrics=result['metrics'], feature_importance=result['feature_importance'], predictions=result['predictions'], task_type=task_info['task_type'])
                progress_bar.progress(90)
                st.session_state.model_result = {
                    'model': result['model'], 'metrics': result['metrics'],
                    'feature_importance': result['feature_importance'], 'predictions': result['predictions'],
                    'charts': charts, 'task_type': task_info['task_type'],
                    'target_column': task_info['target_column'], 'report_generator': report_gen,
...
def show_results_no_code():
    ...
    if task_type == 'classification':
        ...
        with col4: st.metric("MSE", f"{metrics.get('rmse', 0)**2:.2f}")
        st.info(f"🤖 Best Model: **{metrics.get('model_name', 'Unknown')}**")
        r2 = metrics.get('r2_score', 0)
        if r2 >= 0.99: st.warning(f"⚠️ R² is suspiciously perfect ({r2:.4f}). Possible data leakage.")
        elif r2 < 0.5: st.warning(f"⚠️ R² Score is low ({r2:.4f}). Consider checking your features or target column.")
    ...
    if task_type == 'classification':
        ...
        if 'feature_importance' in charts: st.plotly_chart(charts['feature_importance'], use_container_width=True)
        col1, col2 = st.columns(2)
        with col1:
            if 'confusion_matrix' in charts: st.plotly_chart(charts['confusion_matrix'], use_container_width=True)
        with col2:
            if 'metrics_comparison' in charts: st.plotly_chart(charts['metrics_comparison'], use_container_width=True)
    elif task_type == 'regression':
        st.markdown("### 📈 Visualizations")
        col1, col2 = st.columns(2)
        with col1:
            if 'actual_vs_predicted' in charts: st.plotly_chart(charts['actual_vs_predicted'], use_container_width=True)
        with col2:
            if 'residuals' in charts: st.plotly_chart(charts['residuals'], use_container_width=True)
        ...
        if 'predictions' in result: st.dataframe(result['predictions'].head(), use_container_width=True)
        if "viz_data" in result:
            import plotly.express as px
            fig = px.scatter(result["viz_data"], x="pca_1", y="pca_2", color="cluster", title="Cluster Visualization (PCA Projection)")
            st.plotly_chart(fig, use_container_width=True)
    st.markdown("### 📥 Download Results")
    col1, col2 = st.columns(2)
    with col1:
        if "predictions" in result:
            st.download_button(label="📊 Download Predictions CSV", data=result['predictions'].to_csv(index=False), file_name=f"predictions_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv", mime="text/csv", use_container_width=True)
    with col2:
            ...
            if st.button("📄 Generate PDF Report", use_container_width=True):
                with st.spinner("Generating PDF report..."):
                    try:
                        ...
                        dataset_info = {'n_samples': len(st.session_state.uploaded_data), 'n_features': len(st.session_state.uploaded_data.columns), 'target_column': result.get('target_column')}
                        result['report_generator'].generate_pdf_report(output_path=pdf_path, metrics=result['metrics'], feature_importance=result.get('feature_importance', pd.DataFrame()), task_type=task_type, dataset_info=dataset_info, predictions=result.get('predictions'), df=st.session_state.uploaded_data)
                        with open(pdf_path, 'rb') as f:
                            pdf_data = f.read()
                        st.download_button(label="⬇️ Download PDF Report", data=pdf_data, file_name=f"promptml_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf", mime="application/pdf", use_container_width=True)
                        os.unlink(pdf_path)
                    except Exception as e:
...
def show_results_developer():
    ...
    if result['task_type'] == 'classification':
        ...
        with col3: st.metric("Recall", f"{metrics.get('recall', 0):.2%}")
        with col4: st.metric("F1 Score", f"{metrics.get('f1_score', 0):.2%}")
        if metrics.get('accuracy', 0) >= 0.99: st.warning("⚠️ Accuracy is suspiciously high (≥99%). Check if the target column is leaking into features.")
        ...
        r2 = metrics.get("r2_score", 0)
        if r2 >= 0.99: st.warning(f"⚠️ R² is suspiciously perfect ({r2:.4f}). Possible data leakage.")
        elif r2 < 0.5: st.warning(f"⚠️ R² Score is low ({r2:.4f}). Model may not be fitting well.")
    if result.get("comparison") is not None and not result["comparison"].empty:
        with st.expander("📊 View All Models Comparison", expanded=False):
            comp_df = result["comparison"]
            numeric_cols = comp_df.select_dtypes(include="number").columns.tolist()
                ...
                with st.spinner("Generating PDF report..."):
                    try:
                        with tempfile.NamedTemporaryFile(delete=False, suffix='.pdf') as tmp_file:
                            pdf_path = tmp_file.name
                        dataset_info = {'n_samples': len(st.session_state.uploaded_data), 'n_features': len(st.session_state.uploaded_data.columns), 'target_column': result.get('target_column')}
                        result['report_generator'].generate_pdf_report(output_path=pdf_path, metrics=result['metrics'], feature_importance=result.get('feature_importance', pd.DataFrame()), task_type=result['task_type'], dataset_info=dataset_info, predictions=result.get('predictions'), df=st.session_state.uploaded_data)
                        with open(pdf_path, 'rb') as f:
                            pdf_data = f.read()
                        st.download_button(label="⬇️ Download PDF Report", data=pdf_data, file_name=f"promptml_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf", mime="application/pdf", use_container_width=True)
                        os.unlink(pdf_path)
        ...
        if st.button("🔨 Generate Python Package", use_container_width=True, key="dev_pkg_btn"):
            with st.spinner("Creating production package..."):
                try:
                    ...
                    zip_buffer.seek(0)
                    st.download_button(label="⬇️ Download Package (ZIP)", data=zip_buffer.getvalue(), file_name=f"promptml_package_{datetime.now().strftime('%Y%m%d_%H%M%S')}.zip", mime="application/zip", use_container_width=True)
...
README.md            ← Full deploy instructions
    ...
    with st.expander("⚠️ Common Issues & Fixes"):
        ...
        | Problem | Fix |
        |---|---|
        | `No module named 'pycaret'` | Advanced settings → Python version not set to 3.11. Go to ⋮ → Settings → Advanced → Python 3.11 → Reboot |
...
{context_block}"""
        ...
        user_raw_prompt = st.text_input("Paste your prompt to refine:", placeholder="e.g. predict sales, classify churn...", key="prompt_refiner_input")
        if st.button("✨ Refine My Prompt", use_container_width=True, key="refine_btn"):
            if user_raw_prompt.strip():
                refine_question = (f"Refine and improve this ML prompt for use in PromptML Studio. It should clearly mention: what to predict or classify, the target column if obvious, and the task type (regression/classification/clustering). Give 2-3 improved versions, keep them short and natural. My original prompt: '{user_raw_prompt}'")
                st.session_state.chat_history.append({"role": "user", "content": refine_question})
                try:
                    from groq import Groq
                    groq_client = Groq(api_key=os.environ.get("GROQ_API_KEY", st.secrets.get("GROQ_API_KEY", "")))
                    groq_response = groq_client.chat.completions.create(model="llama-3.3-70b-versatile", max_tokens=1024, messages=[{"role": "system", "content": system_prompt}, *[{"role": m["role"], "content": m["content"]} for m in st.session_state.chat_history if m["role"] in ("user", "assistant")]])
        ...
        user_input = st.text_input("Ask me anything...", value=prefill, placeholder="e.g. What is Random Forest? Why is R² negative?", key="chat_input")
